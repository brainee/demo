...
function isQuotMark(content, start) {
  var startCtx = content.substr(0, start);
  var reg = /"/g;
  var startCount = startCtx.match(reg);

  if (startCount && startCtx.length) {
    return startCtx.length % 2 !== 0;
  }

  return false;
}

// function indexReplace(content, start, length, newContent) {
//   content = content.substr(str)
//   return content;
// }
...
module.exports = function(grunt) {
  ...
  grunt.registerTask('web2.0', function() {
    ...
    var requireDefaults = {
      ...
      removeCombined: grunt.option('path') ? true : false,
      ...
      onBuildWrite: function (moduleName, path, contents) {
        ...
        if (moduleName.indexOf('text!') == 0) {
          contents = contents
                      .replace(/(\\r)?\\n\s*/g, '')
                      // html标签前后空格
                      .replace(/\s*<(\w|\/\w)("[^"]*"|'[^']*'|[^>'"])*>\s*/g, function(match) {
                        // 过滤不安全的underscore
                        match = match.trim().replace(/<%([\s\S]+?)%>/g, function(tmp, $1) {
                          $1 = $1.trim().replace(/^([=-])\s+/g, '$1');
                          return escape('<%') + $1 + escape('%>');
                        });

                        return match;
                      })
                      .replace(/\s*<%([\s\S]+?)%>\s*/g, function(match, $1) {
                        $1 = $1.replace(/^([=-])\s+/g, '$1').trim();

                        return '<%' + $1 +'%>';
                      })
                      .replace(/<(\w|\/\w)("[^"]*"|'[^']*'|[^>'"])*>/g, function(match) {
                        match = match.replace(/%3C%25([\s\S]+?)%25%3E/g, '<%$1%>');
                        return match;
                      })
                      .replace(/<!--[\s\S]*?-->/g, '');   // 注释
        }
        ...
        process: function(contents, filepath) {
          ...
          contents = contents.replace(
            ...
            function(tag) {
              ...
              while (item = reg.exec(tag)) {
                if (!isQuotMark(tag, item.index)) {
                  // tag = tag.substr(0, item.index) + '_="' + item[0] +'"' + tag.substr(item.index + item.length);
                  console.log(item);
                }
              }

              // replaces.forEach(function(item) {
              //   tag = tag.replace(item, '_=')
              // });
  ...
  grunt.registerTask('hybrid2.0', function() {
    ...
    var initConfig = {
      compress: {
        zip: {
          ...
          files: [{
            expand: true,
            cwd: path.join('webapp', config.hybridChannel),
            src:  '**/*',
            dest: config.hybridChannel
          }]
        }
      }
    };
    ...
    function getLayOut() {
      var content = file.readFileSync(path.join(webappPath, 'views/_ViewStart.cshtml'), {
        encoding: 'utf8'
      });
      var reg = /"~\/([^"]+)"/;
    ...
    function processCsthml(contents, filepath) {
      ...
      // 替换自定义服务器变量
      if (config.serviceReplace) {
        var serviceReplace = config.serviceReplace[config.buEnv];
        var regStr;

        for (var i in serviceReplace) {
          regStr = i
            .replace(/\(\)/, '([^)]+)')
            .replace(/([\/\.\(\)])/g, '\\$1')
            .replace('[', '([')
          ...
          contents = contents.replace(
            new RegExp(regStr, 'g'),
            function(match, $1) {
              return serviceReplace[i] + $1.replace(/"/g, '');
            }
          );
        }
      }
    ...
    // 生成首页
    if (config.childChannel) {

    } else {
      grunt.file.copy(
        path.join('webapp', config.hybridChannel, getLayOut()),

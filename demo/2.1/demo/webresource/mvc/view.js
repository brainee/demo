 ...
 'cPageView'
  ...
  var executeContext = $([
    '<main class="cm-content">',
      '<h2 class="guide-title">及时调试器</h2>',
      '<div class="form">',
        '<div class="form-item">',
            '<textarea style="width: 90%; height: 200px;" class="cur_js_str" ></textarea>',
        '</div>',
        '<div class="form-item form-item--arrow-btn">',
            '<span class="btn-primary js_run_javascript">运行</span>',
        '</div>',
      '</div>',
    '</main>'
  ].join(''));
  ...
  var View = cPageView.extend({
    ...
    onAfterShow: function () {
      ...
      $('#log').remove();
      if (this.openJsExecuteContext)
        this._initExecuteContext();
      ...
      //解决main.css冲突问题
      var main_css = $('#main_css');
      var global_css = $('#global_css');
      if (Lizard.uibeta_sandbox) {
        main_css.remove();
      }
    ...
    //初始化运行环境框
    _initExecuteContext: function () {
      this.$el.append(executeContext);
      this.events['click .js_run_javascript'] = function () {
        var str = this.$('.cur_js_str').val();
